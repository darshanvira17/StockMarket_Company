...
@RestController
public class TestControllerCompanyF {

	@RequestMapping(value = "/companyF", method = RequestMethod.GET)
	public Company firstPage() {
		Company cmp = new Company();
		cmp.setName("PQR");
		cmp.setNumber_of_shares(12000);
		cmp.setPrice_of_shares(100);
		cmp.setShares_doubles_when(5000);
		return cmp;
	}
	@RequestMapping(value = "/companyF", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
...
//	    ResponseUpgrade response= null;
		if (response != null) {
			...
			try {
				...
				String q="select Number_of_shares from company_f WHERE name='"+response.getSeller()+"'";
				...
				String r="UPDATE company_f SET Number_of_shares="+seller_shares_after+" WHERE name='"+response.getSeller()+"'";
				...
				String s="SELECT Number_of_shares from company_f WHERE name='"+response.getBuyer()+"'";
				...
				String t="UPDATE company_f SET Number_of_shares="+buyer_shares_after+" WHERE name='"+response.getBuyer()+"'";
