...
public class TestControllerCompanyC {
	...
	public Company firstPage() {
		...
		cmp.setName("GHI");
		cmp.setNumber_of_shares(400);
		cmp.setPrice_of_shares(50);
		cmp.setShares_doubles_when(200);
		return cmp;
	}
...
//	    ResponseUpgrade response= null;
		if (response != null) {
			...
			try {
				...
				String q="select Number_of_shares from company_c WHERE name='"+response.getSeller()+"'";
				...
				String r="UPDATE company_c SET Number_of_shares="+seller_shares_after+" WHERE name='"+response.getSeller()+"'";
				...
				String s="SELECT Number_of_shares from company_c WHERE name='"+response.getBuyer()+"'";
				...
				String t="UPDATE company_c SET Number_of_shares="+buyer_shares_after+" WHERE name='"+response.getBuyer()+"'";
