...
@RestController
public class TestControllerCompanyD {

	@RequestMapping(value = "/companyD", method = RequestMethod.GET)
	public Company firstPage() {
		Company cmp = new Company();
		cmp.setName("JKL");
		cmp.setNumber_of_shares(5000);
		cmp.setPrice_of_shares(70);
		cmp.setShares_doubles_when(900);
		return cmp;
	}
	@RequestMapping(value = "/companyD", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
...
//	    ResponseUpgrade response= null;
		if (response != null) {
			...
			try {
				...
				String q="select Number_of_shares from company_d WHERE name='"+response.getSeller()+"'";
				...
				String r="UPDATE company_d SET Number_of_shares="+seller_shares_after+" WHERE name='"+response.getSeller()+"'";
				...
				String s="SELECT Number_of_shares from company_d WHERE name='"+response.getBuyer()+"'";
				...
				String t="UPDATE company_d SET Number_of_shares="+buyer_shares_after+" WHERE name='"+response.getBuyer()+"'";
